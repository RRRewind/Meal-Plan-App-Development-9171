...
const RatingModal = ({ recipe, isOpen, onClose }) => {
  ...
  // ✅ FIXED: Better modal state management with ready state
  useEffect(() => {
    if (isOpen && recipe) {
  ...
  // ✅ FIXED: Clear state when modal closes with proper cleanup
  useEffect(() => {
    if (!isOpen) {
      const timer = setTimeout(() => {
        setRating(0);
  ...
  // ✅ FIXED: Only render if properly open and ready
  if (!isOpen || !recipe || !modalReady) return null;
